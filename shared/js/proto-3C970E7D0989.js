...
$(function () {
	...
	$("#grid").mousemove(function (event) {
	    if (Grid.Draw.canDraw && Grid.hasSet && Grid.Draw.tool == "pencil") {
	        Grid.Draw.pencil(event.pageX, event.pageY);
	    }
	    else if (Grid.Draw.canDraw && Grid.hasSet && Grid.Draw.tool == "brush") {
	...
	$("#gdrawpencil").click(function () { Grid.Draw.prevDiam = Grid.Draw.diameter; Grid.Draw.diameter = 1; $("#gdrawdiam").val("1"); });
	$("#gdrawbrush").click(function () { Grid.Draw.diameter = Grid.Draw.prevDiam; $("#gdrawdiam").val(Grid.Draw.prevDiam); });
	$("#gdrawerase").click(function () { Grid.Draw.colour = "transparent" });
	$("#gdrawdiam").mouseleave(function () {
	    if (!isNaN($("#gdrawdiam").val())) {
	        Grid.Draw.diameter = $("#gdrawdiam").val()
	    }
	});
	$("#gdrawcolour").mouseover(function () {
	    Grid.Draw.colour = $("#drawcolour").val();
	})
...
Calendar.generate = function () {
    ...
    Calendar.cellHeight = parseInt(Calendar.height / Math.ceil(Calendar.monthlength / 7));
    var count = 1;
    for (i = 0; i < 7; i++) {
        modifyHtml("calinner", "<div id='calcol" + i + "' class='calcol'>");
        for (j = 0 ; j < Math.ceil(Calendar.monthlength / 7) ; j++) {
    ...
    for (i = 0; i < 7; i++) {
        var tempWeek = [];
        for (j = Math.ceil(Calendar.monthlength / 7) * i; j < Math.ceil(Calendar.monthlength / 7) * (i + 1) ; j++) {
